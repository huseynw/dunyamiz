    ...
    try {
        ...
        if (type === 'update_config') {
            ...
            if (musicSource === 'cloudinary') {
                ...
                setAdminStatus('Cloudflare R2 üçün imzalı yükləmə hazırlanır...', 'info');

                const coverExt = coverFile ? (coverFile.name.split('.').pop()?.toLowerCase() || 'jpg') : 'jpg';
                ...
                const { rawText: prepRawText, data: prepResult } = await parseAdminApiResponse(prepResponse);
                if (!prepResponse.ok || !prepResult.success) {
                    throw new Error(buildAdminRequestError(prepResponse, prepResult, prepRawText));
                }
                ...
                if (!audioPut.ok) {
                    ...
                    throw new Error(`R2 audio upload xətası (${audioPut.status}): ${raw || audioPut.statusText}`);
                }
                ...
                if (coverFile && prep.coverUploadUrl) {
                    ...
                    if (!coverPut.ok) {
                        ...
                        throw new Error(`R2 cover upload xətası (${coverPut.status}): ${raw || coverPut.statusText}`);
                    }
                }
    ...
    try {
        ...
        if (bytes.length >= 10 && String.fromCharCode(...bytes.slice(0, 3)) === 'ID3') {
            ...
            while (offset + 10 <= bytes.length && offset < 10 + tagSize) {
                ...
                if (frameId === 'TIT2') fallback.title = decodeId3TextFrame(frameContent) || fallback.title;
                if (frameId === 'TPE1') fallback.artist = decodeId3TextFrame(frameContent) || fallback.artist;
...
async function fillMusicMetadataFromSelectedFile(forceOverwrite = false) {
    ...
    if (!file || !titleInput || !artistInput) {
        if (statusEl) statusEl.textContent = 'Əvvəl MP3 seç. Sistem əvvəl ID3 tag-a, tapa bilməsə fayl adına baxacaq.';
        return;
    }
    ...
    if (!forceOverwrite && hasManualTitle && hasManualArtist) {
        if (statusEl) statusEl.textContent = 'Title və artist artıq doludur. İstəsən düyməyə basıb yenidən çıxarda bilərsən.';
        return;
    }
    ...
    if (statusEl) {
        if (meta.title || meta.artist) {
            ...
            statusEl.textContent = 'Metadata tapılmadı. Fayl adını və ya ID3 tag-ları yoxla.';
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    musicFileInput?.addEventListener('change', async () => {
        ...
        if (file) {
            ...
            const statusEl = document.getElementById('admin-auto-metadata-status');
            if (statusEl) {
                statusEl.textContent = 'Əvvəl MP3 seç. Sistem əvvəl ID3 tag-a, tapa bilməsə fayl adına baxacaq.';
            }
        }
    });
