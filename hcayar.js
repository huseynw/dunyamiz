...
function buildAdminRequestError(response, result = {}, rawText = '') {
    ...
    const mainDetail = result?.error || result?.message || result?.details?.error || result?.details?.message || result?.stack || result?.rawText || toPlainErrorText(rawText);
    const lines = [
    ...
    try {
        ...
        if (type === 'update_config') {
            ...
            if (musicSource === 'cloudinary') {
                ...
                setAdminStatus('Cloudflare R2 üçün fayllar hazırlanır...', 'info');
                ...
                requestPayload = {
                    ...
                    r2AudioKey: `music/${slug}.mp3`,
                    audioContent: audioBase64,
                    audioContentType: audioMime,
                    r2CoverKey: coverFile ? `covers/${slug}.${coverExt}` : '',
                    coverContent: coverBase64,
                    coverContentType: coverFile ? coverMime : ''
                };
